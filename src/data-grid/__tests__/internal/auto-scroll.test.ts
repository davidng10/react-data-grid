...
describe("edgeScrollDelta", () => {
  it("is zero in the interior (no edge near the pointer)", () => {
    expect(edgeScrollDelta({ x: 500, y: 300 }, el(), insets)).toEqual({
      dx: 0,
      dy: 0,
    });
  });
  ...
  it("ramps negative near the left band and positive near the right band", () => {
    expect(edgeScrollDelta({ x: 100, y: 300 }, el(), insets).dx).toBe(
      -EDGE_SPEED
    );
    expect(edgeScrollDelta({ x: 920, y: 300 }, el(), insets).dx).toBe(
      EDGE_SPEED
    );
  });

  it("ramps negative near the top band and positive near the bottom", () => {
    expect(edgeScrollDelta({ x: 500, y: 40 }, el(), insets).dy).toBe(
      -EDGE_SPEED
    );
    expect(edgeScrollDelta({ x: 500, y: 580 }, el(), insets).dy).toBe(
      EDGE_SPEED
    );
  });
  ...
  it("insets move the band inward — a point inside the frozen band triggers a scroll", () => {
    ...
    expect(edgeScrollDelta({ x: 130, y: 300 }, el(), insets).dx).toBe(
      -EDGE_SPEED
    );
    expect(
      edgeScrollDelta({ x: 130, y: 300 }, el(), { left: 0, right: 0 }).dx
    ).toBe(0);
  });

  it("omitting top disables vertical scroll (horizontal-only gesture)", () => {
    const horizontalOnly = { left: 100, right: 50 };
    expect(edgeScrollDelta({ x: 500, y: 40 }, el(), horizontalOnly)).toEqual({
      dx: 0,
      dy: 0,
    });
    expect(edgeScrollDelta({ x: 500, y: 580 }, el(), horizontalOnly).dy).toBe(
      0
    );
    expect(edgeScrollDelta({ x: 100, y: 580 }, el(), horizontalOnly).dx).toBe(
      -EDGE_SPEED
    ); // x still works
  });

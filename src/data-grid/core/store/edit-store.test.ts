...
describe('editStore', () => {
  ...
  it('begin opens an editor with the initial draft', () => {
    const s = createEditStore()
    s.begin(CELL, 'hello')
    expect(s.getSnapshot()).toEqual({ status: 'editing', cell: CELL, draft: 'hello' })
  })
  ...
  it('submitting transitions from editing and keeps cell + draft', () => {
    ...
    s.begin(CELL, 'v')
    s.submitting()
    expect(s.getSnapshot()).toEqual({ status: 'submitting', cell: CELL, draft: 'v' })
  })
  ...
  it('typing after an error clears the error back to editing (retry-by-typing)', () => {
    ...
    s.fail('nope')
    s.setDraft('v2')
    expect(s.getSnapshot()).toEqual({ status: 'editing', cell: CELL, draft: 'v2' })
  })
